...
name rules allow) cannot make the backend write into a system directory.
"""
...
from paw_backend.repositories.limits import DEFAULT_MIN_LINUX_UID
...
class LoginNameAccountDirectory:
    ...
    ``lookup`` (default ``pwd.getpwnam``) is the account database; a test
    replaces it. The lookup runs in a thread (it can block on NSS / LDAP).
    """
    ...
    def __init__(
        ...
        *,
        ...
        if isinstance(min_uid, bool) or not isinstance(min_uid, int) or min_uid < 1:
            raise TypeError("min_uid must be a positive int")
