...
def visible(version: Any, scopes: ResolvedScopes, now: datetime) -> ColumnElement[bool]:
    ...
    * freshness: ``session_only`` memories are not long-term memory, and an
      ...
      ``stale`` memories stay: they are lowered and flagged, or excluded, by the
      caller's ``stale_policy``.
    """
    return and_(
...
def keyword_statement(
    scopes: ResolvedScopes, now: datetime, tsquery: str, limit: int
) -> Select[Any]:
    ...
    return (
        ...
        .where(visible(version, scopes, now), document.op("@@")(query))
...
def relations_statement(
    scopes: ResolvedScopes, now: datetime, version_ids: Collection[UUID], limit: int
) -> Select[Any]:
    """Conflict and supersede relations of ``version_ids`` whose BOTH ends are readable.

    A relation to a memory the caller may not read is not returned: neither its
    existence nor the other end is ever visible.
    """
    ...
    return (
        ...
        .where(
            MemoryRelation.relation_type.in_([CONFLICT, SUPERSEDES]),
            or_(
                MemoryRelation.from_version_id.in_(ids),
                MemoryRelation.to_version_id.in_(ids),
            ),
            visible(newer, scopes, now),
            visible(older, scopes, now),
        )
        .order_by(
            MemoryRelation.from_version_id,
            MemoryRelation.to_version_id,
...
def versions_statement(
    scopes: ResolvedScopes, now: datetime, version_ids: Collection[UUID]
) -> Select[Any]:
    """The readable active versions among ``version_ids``."""
    version = aliased(MemoryVersion, name="mv")
    return select(*_columns(version)).where(
        version.id.in_(sorted(version_ids)), visible(version, scopes, now)
    )
