...
administration of Shared Memory, PAW-046) and ``journal/`` (the Immediate Journal
and the background consolidation, PAW-041). There are no other repositories or
services here: conflict handling (PAW-042), retrieval (PAW-043) and Markdown
projection (PAW-045) come later and build on these tables. See ``models.py`` for
the layout and ``acl.py`` for how permissions are filtered in SQL.
