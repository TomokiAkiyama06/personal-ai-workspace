...
├─ migrations/             # env.py と Revision（0001 は空の Baseline、0021 は users / setup_tokens、0026 は Project、0031 は Tool Approval、0033 は Queue / Budget / Loop、0040 は Memory Schema、0046 は Shared Memory Candidate、0050 は Research Scratch、0052 は Evidence / Claim Provenance、0083 は `tasks (project_id, state)` の Index）
├─ paw_backend/
...
- **Task `input` の検証**（`TaskService.create_task`）: `input` は JSON Object で、`json.loads` が返す型（`dict`〔キーは `str`〕、`list`、`str`、`int`、`float`、`bool`、`None`）だけを受け付けます。整数キーや `tuple` などを黙って変換して保存することはしません。次のものは、DB へ書く前に `InvalidCommandArgumentError` で拒否します（エラー文に値は含めません）。
  ...
  検査と保存する Copy は、呼び出し元の Object を **1 回だけ走査する同じ処理**で作ります（`_JsonInputCheck.check_object` が検査しながら Copy を返し、エンコードと保存はその Copy だけを使います）。別の Thread が検査の後に呼び出し元の Object を書き換えても（`list` を `tuple` や整数キーの `dict`、巨大な値に置き換えるなど）、検査していない値が保存されたり、上限を超えた量の処理をエンコードが行ったりすることはありません。走査の途中で `dict` の大きさが変わった場合も、`RuntimeError` ではなく `InvalidCommandArgumentError` で拒否します（`tests/test_task_input_snapshot.py`）。
...
[Decision 0008](../../docs/decisions/0008-project-membership-and-lifecycle-policy.md) の 8（2026-09-25 に Human が承認）の Gate です。Task Lane の `create_task`・Retry・Restart と、Queue Lane（PAW-033）の `TaskQueue.enqueue` は、**新しい作業を受け入れる 4 つの操作**です。
...
- **動き。** `TaskService(database, project_gate=...)` と `TaskQueue(database, project_gate=...)` が `ProjectGate`（Protocol。`paw_backend/tasks/project_gate.py`）を受け取ると、その操作の書き込みと **同じ Transaction の中**で `require_active(session, project_id)` を呼びます。実装（`paw_backend/projects/task_gate.py` の `ProjectStateGate`）は、Project の行を `SELECT status ... FOR SHARE` で Lock し、Active でなければ `ProjectNotActiveError`（`project_not_active`。固定文言で、Project も状態も書かない）を送出します。**何も書きません**（Task の行も、履歴も、Entry も）。Lock は書き込みの Transaction が終わるまで続きます。
...
- **判定。** Active 以外（Archived、Pending deletion、Deleted）は全て拒否し、存在しない Project も同じ Error です（既定は拒否）。Retry と Restart は、不正な遷移の判定（`IllegalTransitionError`）の後、Task の行の Lock を取った後で Gate を呼びます。`enqueue` は Task の ID しか受け取らないので、Task の `project_id`（変わらない列）を読んでから Gate を呼びます（存在しない Task は `TaskNotFoundError`）。
- **Gate を通さないもの。** Cancel、Fail、Stop Now、Pause、Start、Resume など他の Command と、`claim_next`・`heartbeat`・`release`・`complete`・`cancel` は Project を見ません（削除待ちの Project の Task を止める Processor 自身が Cancel を発行します）。Archive の時点で受け入れ済みの作業の扱いは未決です（[Decision 0020](../../docs/decisions/0020-project-state-gate.md)（Proposed）の C）。
...
- **条件つきの Queue の取消。** `TaskQueue.cancel(task_id, only_if_task_terminal=True)` は、Task の行を `SELECT ... FOR SHARE` で Lock し、Task が Completed / Failed / Cancelled のときだけ Entry を Cancel します。「Task が終わったので Entry は無駄」という判断が、Entry を Cancel する前に古くならないようにするためです（先に Commit された Restart の Entry は残り、後の Restart は取消の Commit まで待ちます）。`cancel_in(session, task_id, ..., only_if_task_terminal=...)` は同じことを呼び出し側の Transaction で行い、Commit も Rollback もしません。Test: `tests/test_queueing_conditional_cancel.py`。Queue が `tasks` を読むのは、この 2 か所（`enqueue` の `project_id` と条件つきの取消の `state`）だけで、`tasks` を書くことはありません。

Test: `tests/test_project_state_gate.py`（4 つの操作 × Active 以外の状態、書き込みが残らないこと、Lock が Commit まで続くこと、有界の待ちと `lock_timeout` の復元、引数、Delete と書き込みの両方の順序の競合）、`tests/test_tasks_project_index.py`（Index の Migration の Up / Down / Up と Autogenerate の差分なし、Plan）。

Migration `0083`（Index だけ。Table を作らないので権限の変更はありません）の `down_revision` は `0026` です（鎖は `0001 → 0025 → 0032 → 0040 → 0021 → 0033 → 0031 → 0050 → 0046 → 0052 → 0026 → 0083`。統合時に並べ替えます）。
...
- Queue は `tasks` を変更しません。読むのは 2 か所だけです（Issue #83）: `enqueue` が Gate のために Task の `project_id` を、`cancel(only_if_task_terminal=True)` が Task の `state` を読みます（どちらも省略できる機能です）。Orchestrator が `claim_next` と PAW-032 の `start` を組み合わせます。
...
Test（人の判断）: `tests/test_tools_approvals.py` の `DecisionDeadlineTest`（応答しない Store、1 つの期限、Step-up は数えないこと、型名だけの Log）、`tests/test_tools_postgres.py` の `StalledServerTest`（応答しない Server）、`DecisionStatementsShareOneDeadlineTest`（Statement ごとの残り時間）、`DecisionDeadlineTest`（行の Lock で Statement を止め、期限で `unavailable` を返すこと。Statement が Server に届いて Lock を待っていることを `pg_stat_activity` で確かめてから期限を待ちます。放棄した Statement の結果は、時間の余裕ではなく順序で決まる 2 つの場合を別々の Test にしています。期限の後に行の Lock を放して反映させる場合（Statement を実行した Server の Process が終わってから状態を読み、承認と履歴が揃うこと、呼び直すと `not_pending` / `not_open` が返ることを確かめます。Lock の待ちが無くなった時点では Commit の途中でありうるため、そこでは読みません）と、接続を確立できず Statement が Server に届かない場合（承認は `pending` のまま、呼び直すと決定できます）です）。
...
- `published_at` は `None` か、UTC に変換できる Timezone つきの `datetime` だけです。まず標準の `datetime` の Method で Field を通常の `datetime` へ複製し（`datetime.astimezone` は途中の値を Subclass 自身の Constructor で作るため、複製せずに呼ぶと Adapter の Code が動きます）、その複製を UTC へ変換して、通常の UTC の `datetime` にします。変換は `astimezone` ではなく、1 回目に読んだ `utcoffset` の値で行い、2 回目の読み取りが 1 回目と違う `tzinfo`（`+01:00` の次に `+02:00` を返すなど、呼び出しごとに答えが変わるもの）は不正な Response です（1 回目の検査と 2 回目の変換で別の Offset が使われ、時刻が黙って 1 時間ずれるのを防ぎます。`tests/test_research_published_offset.py`）。動くのは Adapter の `tzinfo.utcoffset` だけで、それが出した例外は、`asyncio.CancelledError`、`KeyboardInterrupt`、`SystemExit`、`GeneratorExit` を含む `BaseException` の全てが不正な Response です（同期の Code に Task の Cancel は届かないため。[Decision 0012](../../docs/decisions/0012-research-provider-adapter-policy.md) の 10）。`utcoffset` が `asyncio.current_task().cancel()` を呼んで Offset を返す場合も同じで、Broker が Response の検証の前後で `Task.cancelling()` を比べ、増えた分を `Task.uncancel()` で取り消して、`invalid_response` にします。
...
- **Delete 開始の後に作られた Task と Entry（Decision 0008 の 8 の 4）は、Gate が閉じます。** `create_task`・Retry・Restart・`enqueue` は、Project の状態 Gate（[Project の状態 Gate](#project-の状態-gateissue-83)）を渡した Service では、書き込みと同じ Transaction で Project の行を `FOR SHARE` で Lock し、Active 以外を拒否します。Delete 開始（`FOR UPDATE`）と直列になるため、Delete が Commit された後には Task も Entry も作れず、Commit される前の書き込みは Processor が見つけて止めます。
  **Gate を渡さずに組み立てた Service には効きません**（Project のない Test と Tool のため、省略できます。本番の組み立ては PAW-034 が行い、必ず渡します。[Decision 0020](../../docs/decisions/0020-project-state-gate.md)（Proposed）の A）。そのため Orchestrator は Pending deletion の Project にも通常の周期で `stop_project_tasks` を呼んでください（安価で冪等です。Gate があれば正しさに必須ではなく、なければ Processor の再実行が Task と Entry を止めます。Test: `test_a_task_created_after_the_deletion_began_is_stopped_on_a_rerun`、`test_an_entry_of_a_finished_task_is_found_by_project_on_a_rerun`）。Orchestrator が呼ぶことは、PAW-034（[#30](https://github.com/TomokiAkiyama06/personal-ai-workspace/issues/30)）の受け入れ条件に追記済みです。
...
- **残る制限。** Queue は、終了した Task の Entry を Claim のときに飛ばしません（Issue #83 の受け入れ条件はその Claim か条件つきの Cancel のどちらかで、条件つきの Cancel と 1 つの Transaction を選びました）。ユーザーの Cancel や Worker の Fail が終了した Task に残した active な Entry は、Sweep（削除中の Project）、Orchestrator（PAW-034）の掃除、または Restart が拾うまで残ります。Archive の時点で受け入れ済みの Task の扱い（Claim と Start は Project を見ません）は未決です（[Decision 0020](../../docs/decisions/0020-project-state-gate.md)（Proposed）の C）。
