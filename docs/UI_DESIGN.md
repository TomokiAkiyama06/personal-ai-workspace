...
User削除時は30日後の完全消去対象にRecovery履歴・復旧用コピーが含まれることと、復元不能になる範囲を承認画面へ表示する。
期限到達後の消去が未完了の場合は完全削除済みと表示せず、`Pending deletion` と期限未達・失敗・再試行状態をOwnerへ表示する。
Provider側の保持コピー等を含め、消去を確認できていない範囲は完了表示しない。
