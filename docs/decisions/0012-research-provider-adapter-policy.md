...
10. **同期で動く Adapter の Code が出した `BaseException` は、Adapter 自身の失敗とする。** `asyncio.CancelledError` は `BaseException` で、`except Exception` では捕まらない。`Task.cancel()` は `await` の地点でしか届かないので、`await` を挟まずに動く Adapter の Code（`published_at` の `tzinfo.utcoffset`、`provider.search` / `provider.fetch` を読んで呼ぶ部分。Property や `__getattribute__` を含む）が `CancelledError` を出しても、それは Broker の Task の Cancel ではない。それを通すと `gather()` / `fetch()` 全体が Cancel され、成功した他の Provider の結果が失われる（1 つの Provider が全体を止められる）。
   ...
   **限界:** `Task.uncancel()` は「次の `await` で Cancel する」印を、数が 0 になったときにしか消さない。窓の前に数が 1 以上あって、その Cancel が届いていない Task（`CancelledError` を握りつぶして `uncancel()` を呼ばなかった Task）では、Adapter が付けた印が残り、次の `await` で届く。窓の中で Adapter 自身が `Task.uncancel()` を呼んで数を減らしても検出しない。
   **登録の窓も同じ扱いにする。** `ProviderRegistry.register` と `validate_provider` が Adapter の属性（`name`、`kind`、`search`、`fetch`）を読む部分と、`search` / `fetch` が返した Object を `inspect`（`iscoroutinefunction`、`signature`、`Signature.bind`）で調べる部分も、Adapter の Code（Property、`__getattribute__`、`__getattr__`、Descriptor、Metaclass の Hook、`__class__`、`__signature__`）が同期で動く窓である。読み取りと検査は Member ごとに 1 つの窓にする。
   - Adapter の Code が出した例外は、`BaseException`（`CancelledError`、`KeyboardInterrupt`、`SystemExit`、`GeneratorExit` を含む）を全て、その Member の固定の `ProviderInterfaceError(<member>)` にする（文言は固定で、Adapter の文言と例外 Object は含めない）。`try` の外で送出するので、`__cause__` / `__context__` にも Adapter の例外は付かない（`except` の中で送出すると `__context__` に残り、`logger.exception` の出力に Adapter の文言が入る。`from None` は表示を隠すだけで `__context__` は消さない）。
