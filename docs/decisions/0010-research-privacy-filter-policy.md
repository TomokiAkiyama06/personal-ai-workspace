...
| 4 | Private Host | Host だけの Token が Private なら消す。Port、Path、User 情報（`admin@10.0.0.5`）、IPv6 の `[...]` と Zone ID（`[fe80::1%eth0]:8080`）、末尾の Dot の付いた絶対名（`db.internal.:5432`）、`[]` の無い IPv6（`fd00::` のように `::` で終わる圧縮形を含む）、IPv6 の Network（CIDR 形式の `fd12:3456:789a::/48`）、`%` を含む名前（`db.internal%eth0`、`db.%69nternal`、`db%2einternal`）も対象で、判定の前にそれらを取り除く。Dot を含まない名前は単語と区別できないので残す |
