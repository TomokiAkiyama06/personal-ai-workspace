...
一方で、次の証拠から、これを PAW-032 の完了条件として黙って実装することは、この Decision が承認されるまで見送る。
...
4. **Write 範囲の強制は Tool Broker（PAW-031）が行い、Working Set は呼び出し側が解決して渡す入力である。**
   Decision 0006（PAW-031 の PR に含まれる `docs/decisions/0006-tool-broker-policy.md`、Proposed）は、Orchestrator が Task の作業対象を `TaskScope.repositories` として呼び出しごとに作ると選んでいる。
...
2. **Repository の登録（PAW-027）ができた後、上の未決の項目が決まってから、専用の Migration で追加する。** 実装する Issue は人間が決める（上の 6）。
3. **承認された場合に想定する形**（この形に縛られることは、まだ決めない）:
...
- **A. PAW-032 に最小の Working Set（Repo の UUID と役割だけの Table）を今入れる。** 見送る。要件が復元を求める中身は、Repo ごとの Git 状態と Write 範囲の境界であり、
  ...
  承認前に形を固めると、後で単位や役割の意味が変わったときに、Table を作り直す Migration が要る。
